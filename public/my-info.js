...
(() => {
  ...
  window.renderRecommendationQuota=quota=>{
    ...
    usage.innerHTML=`<span>서재 추가 <b>${quota.used} / 30권</b></span><span>EXP 적립 <b>${number(quota.xp)} / 4권</b></span><small>추천 이용은 무제한 · 서재 추가 현황은 한국 시간 자정 초기화</small>`;
  };
  ...
  function render(){
    ...
    daily.innerHTML=`<div class="daily-heading"><div><span class="step">TODAY'S LIBRARY</span><h3>오늘 나의 서재에 담은 책</h3></div><span>한국 시간 자정 초기화</span></div><div class="daily-overview"><div class="daily-xp"><span>EXP가 쌓인 책</span><strong>${p.quota.xp}<small> / 4권</small></strong><em>권당 +1 EXP</em></div><div class="daily-books"><span>오늘 추가한 책</span><strong>${p.quota.used}<small> / 30권</small></strong><em>${p.quota.remaining}권 더 추가 가능</em></div></div><div class="daily-stamps" aria-label="서재 추가 경험치 4권 중 ${p.quota.xp}권 적립">${Array.from({length:4},(_,i)=>`<span class="${i<p.quota.xp?'is-earned':''}">${i<p.quota.xp?'✓':'+1'}<small>EXP</small></span>`).join('')}</div><div class="daily-book-track" role="progressbar" aria-label="오늘 나의 서재에 추가한 책" aria-valuemin="0" aria-valuemax="30" aria-valuenow="${p.quota.used}"><span style="width:${Math.min(100,p.quota.used/30*100)}%"></span></div><p>${p.quota.remaining===0?'오늘의 서재 추가 한도에 도달했어요. 추천은 계속 이용할 수 있고, 자정부터 다시 담을 수 있어요.':p.quota.remainingXp>0?`책을 서재에 담으면 +1 EXP · 오늘 ${p.quota.remainingXp}권 더 EXP를 받을 수 있어요.`:'오늘 받을 수 있는 서재 추가 EXP를 모두 모았어요. 책은 계속 담을 수 있어요.'}<br>추천 도서를 둘러보는 횟수에는 제한이 없어요.</p>`;
