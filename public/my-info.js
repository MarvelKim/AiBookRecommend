...
(() => {
  ...
  function render(){
    if(!current)return;
    const p=current;q('#headerGudongi').hidden=false;q('#headerGudongi').innerHTML=`${art(p.appearance,true)}<span>구동이 <b>${p.max?'LV MAX':`LV ${p.level}`}</b></span>`;q('#headerGudongi').setAttribute('aria-label',`${p.appearance.name}, 레벨 ${p.level}. 나의 구동이 열기`);
    ...
    daily.innerHTML=`<h3>오늘의 추천</h3><div><span>추천 경험치</span><b>(${Number(p.quota.xp)}/4)</b></div><div><span>추천 도서</span><b>(${p.quota.used}/30권)</b></div><p>${p.quota.remainingXp>0?`새 추천으로 경험치를 ${p.quota.remainingXp}번 더 받을 수 있어요.`:'오늘의 추천 경험치를 모두 받았어요.'}<br>한국 시간 자정 초기화 · 기존 후보 넘기기는 추가 적립되지 않아요.</p>`;
  ...
  window.applyGudongi=p=>{if(!p||!eligible()||p.userId!==state.user)return;current=p;owner=state.user;render();maybeNotify();};
