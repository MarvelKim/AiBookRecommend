...
(() => {
  ...
  let current=null,owner='',refreshVersion=0,activeTab='shelf',notificationPending=false,crop=null,drag=null;
  const number=value=>Number(value||0).toFixed(2),eligible=()=>state.user!=='guest'&&!state.isAdmin;
  ...
  document.body.insertAdjacentHTML('beforeend',`
    ...
    <dialog class="my-dialog" id="mySettingsDialog" aria-labelledby="mySettingsTitle">
      ...
      <section class="my-avatar-settings"><div class="my-avatar" id="settingsAvatar"></div><div><h3 id="settingsUser"></h3><p>나를 보여주는 프로필 사진</p><button type="button" class="refresh" id="chooseAvatar">사진 변경</button><button type="button" class="my-text-button" id="removeAvatar">기본 이미지로</button><input type="file" id="avatarFile" accept="image/jpeg,image/png,image/webp" hidden></div></section>
      ...
      <div class="my-account-actions"><button type="button" class="refresh" id="settingsLogout">로그아웃</button><div><p>회원탈퇴 시 서재와 활동 기록이 함께 삭제됩니다.</p><button type="button" class="my-danger" id="settingsDelete">회원탈퇴</button></div></div>
    </dialog>
  ...
  window.renderRecommendationQuota=quota=>{
    if(!eligible()){usage.textContent='로그인하고 추천을 받으면 구동이도 함께 자라요.';return;}
    if(!quota){usage.textContent='오늘의 추천 사용량을 확인하고 있어요…';return;}
    usage.innerHTML=`<span>오늘 추천 <b>${quota.used} / 30권</b></span><span>추천 EXP <b>${number(quota.xp)} / 4</b></span><small>한국 시간 자정 초기화 · 후보를 다시 보는 것은 차감되지 않아요.</small>`;
  };
  ...
  function render(){
    ...
    q('#gudongiContent').innerHTML=`<div class="gudongi-layout"><article class="gudongi-growth-card"><div class="gudongi-card-top"><span class="step">MY GUDONGI</span><span class="gudongi-level">${p.max?'LV MAX':`LV ${p.level}`}</span></div><div class="gudongi-stage">${art(p.appearance)}<span class="gudongi-orbit" aria-hidden="true"></span></div><div class="gudongi-intro"><p>매일의 한 페이지가 만드는 작은 성장</p><h2>${escapeHtml(p.appearance.name)}</h2><button type="button" class="refresh" id="changeAppearance">모습 바꾸기 <span aria-hidden="true">↗</span></button></div><div class="gudongi-progress"><div><b>${p.max?'최고 레벨에 도착했어요!':`다음 성장까지 ${number(p.remainingXp)} EXP`}</b><span>EXP (${Number(number(p.currentXp))}/${Number(number(p.requiredXp))})</span></div><div class="xp-segments" role="progressbar" aria-label="현재 레벨 경험치" aria-valuemin="0" aria-valuemax="${p.requiredXp}" aria-valuenow="${Math.min(p.currentXp,p.requiredXp)}" aria-valuetext="${Number(number(p.currentXp))}/${Number(number(p.requiredXp))} EXP">${Array.from({length:5},(_,group)=>`<span class="xp-group">${Array.from({length:p.requiredXp/5},(_,unit)=>{const fill=p.max?1:Math.max(0,Math.min(1,p.currentXp-(group*p.requiredXp/5+unit)));return `<i class="xp-unit${fill===1?' filled':''}" style="--xp-fill:${fill*100}%"></i>`;}).join('')}</span>`).join('')}</div><p>${p.max?'최고 레벨에서도 경험치와 독서 기록은 계속 쌓여요.':'조금씩, 꾸준히. 구동이와 나만의 속도로 읽어요.'}</p></div></article><aside class="gudongi-side"><div class="gudongi-stats"><div><span>누적 경험치</span><strong>${number(p.totalXp)} <small>EXP</small></strong></div><div><span>함께한 모습</span><strong>${p.unlockedCount} <small>/ 9</small></strong></div></div><article class="gudongi-guide"><span class="step">GROW TOGETHER</span><h3>책과 기록으로 자라는 구동이</h3><div><i>01</i><section><h4>오늘의 책 만나기 <b>+1 EXP</b></h4><p>1권 이상 제공된 추천 1회당 적립<br>하루 최대 4 EXP · 추천은 30권까지</p></section></div><div><i>02</i><section><h4>챌린지에 발자국 남기기</h4><p>독서·필사 기록의 이번 달성량 × 3 EXP<br>챌린지 경험치는 일일 상한이 없어요.</p></section></div><button type="button" class="recommend-button" id="gudongiChallenge">챌린지 기록하러 가기 <span>→</span></button></article><div class="gudongi-next"><span class="step">NEXT CHAPTER</span><p>${p.max?'모든 모습이 열렸어요. 오늘의 기분에 맞는 구동이를 골라보세요.':`LV ${p.level+1}에서 새로운 모습이 기다리고 있어요.`}</p><small>달성량 수정·삭제 시 경험치와 해금 상태도 함께 반영됩니다.</small></div></aside></div>`;
    const daily=document.createElement('section');daily.className='gudongi-daily';daily.setAttribute('aria-label','오늘의 추천 사용량');daily.setAttribute('aria-live','polite');
    daily.innerHTML=`<div class="daily-heading"><h3>오늘의 추천</h3><span>매일 자정 새롭게</span></div><div class="daily-xp"><div><span>추천으로 키운 구동이</span><strong>+${Number(p.quota.xp)} <small>EXP</small></strong></div><b>${p.quota.xp}회 적립 <span>· 하루 4회</span></b></div><div class="daily-stamps" aria-label="추천 경험치 4회 중 ${p.quota.xp}회 적립">${Array.from({length:4},(_,i)=>`<span class="${i<p.quota.xp?'is-earned':''}">${i<p.quota.xp?'✓':'+1'}<small>EXP</small></span>`).join('')}</div><div class="daily-books"><span>오늘 받은 도서</span><strong>${p.quota.used}<small> / 30권</small></strong></div><div class="daily-book-track" role="progressbar" aria-label="오늘 추천 도서" aria-valuemin="0" aria-valuemax="30" aria-valuenow="${p.quota.used}"><span style="width:${Math.min(100,p.quota.used/30*100)}%"></span></div><p>${p.quota.remaining===0?'오늘의 추천 도서를 모두 받았어요. 자정 이후 다시 만나요.':p.quota.remainingXp>0?`새 추천을 받으면 +1 EXP · 오늘 ${p.quota.remainingXp}회 더 적립할 수 있어요.`:'오늘의 추천 경험치를 모두 받았어요.'}<br>이미 받은 후보를 넘길 때는 추가 적립되지 않아요.</p>`;
    q('.gudongi-stats').after(daily);
    q('#changeAppearance').onclick=openAppearance;q('#gudongiChallenge').onclick=()=>showView('challenges');
  }
  ...
  window.applyGudongi=p=>{if(!p||!eligible()||p.userId!==state.user)return;refreshVersion++;current=p;owner=state.user;render();maybeNotify();};
  ...
  all('.my-dialog').forEach(dialog=>dialog.addEventListener('close',()=>setTimeout(maybeNotify,0)));
  q('#levelUpConfirm').onclick=()=>q('#levelUpDialog').close();
  q('#mySettingsButton').onclick=()=>{if(!eligible())return;render();q('#passwordChangeForm').reset();all('.my-password-form .my-error').forEach(el=>el.textContent='');q('#passwordStatus').textContent='';q('#avatarError').textContent='';q('#mySettingsDialog').showModal();};
  ...
  function openAppearance(){
    ...
    q('#appearanceGrid').innerHTML=(current.appearanceCatalog||current.appearances).map(a=>{
      ...
      return `<button type="button" class="appearance-choice${locked?' is-locked':''}" data-appearance="${a.id}" data-locked="${locked}" aria-pressed="${selected}" ${locked?'disabled':''}><div>${art(a,true)}</div><small>LV ${a.level} · ${a.kind==='bonus'?'보너스 모습':'기본 모습'}</small><b>${escapeHtml(a.name)}</b><span>${locked?`🔒 LV ${a.level}에서 해금`:selected?'현재 모습':'이 모습으로 선택'}</span></button>`;
    }).join('');
    q('#appearanceDialog').showModal();q('[data-appearance][aria-pressed="true"]')?.focus();
  }
