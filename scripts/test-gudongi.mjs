...
const deletedChallengeHistory=await(await f.api('/api/account/xp-history',null,cookie)).json(),deletedChallenge=deletedChallengeHistory.history.find(item=>item.title==='테스트 도서 기록');assert.equal(deletedChallenge.type,'challenge');assert.equal(deletedChallenge.targetId,'');assert.equal(deletedChallenge.context,'함께 읽는 챌린지');assert.equal(deletedChallenge.xp,0);assert.equal(deletedChallenge.deleted,true);assert.ok(deletedChallenge.createdAt>0);
...
const removed=await(await favorite(0,false)).json();assert.equal(removed.removedXp,1);assert.equal(removed.gudongi.totalXp,4);assert.equal(removed.gudongi.level,1);const deletedBookHistory=await(await f.api('/api/account/xp-history',null,cookie)).json();assert.equal(deletedBookHistory.history.some(item=>item.title==='서재 도서 0'&&item.xp===0&&item.deleted),true);const readded=await(await favorite(0)).json();assert.equal(readded.newlyAdded,true);assert.equal(readded.earnedXp,1);assert.equal(readded.gudongi.totalXp,5);assert.equal(readded.quota.used,6);
f.storage.transactionSync(()=>createGudongiSchema(f.sql));const history=await(await f.api('/api/account/xp-history',null,cookie)).json();assert.equal(history.history.filter(item=>item.type==='library').length,5);assert.equal(history.history.some(item=>item.title==='서재 도서 0'&&item.context==='나의 서재'&&!item.deleted),true);
...
for(const table of ['gudongi_profiles','gudongi_xp','gudongi_deliveries','gudongi_library_additions','gudongi_audit','gudongi_xp_deletions'])assert.equal(f.sql.exec(`SELECT COUNT(*) n FROM ${table} WHERE user_id=?`,'reader')[0].n,0);
