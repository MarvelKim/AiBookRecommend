...
try{
  ...
  await evaluate(`document.querySelector('#headerLibrary').click()`);assert.equal(await evaluate(`document.querySelector('#myLibraryTab').getAttribute('aria-selected')`),'true');
  await evaluate(`document.querySelector('#headerGudongi').click()`);assert.equal(await evaluate(`document.querySelector('#myGudongiTab').getAttribute('aria-selected')`),'true');
  await evaluate(`document.querySelector('#profileButton').click()`);assert.equal(await evaluate(`document.querySelector('#mySettingsDialog').open`),true);
  assert.equal(await evaluate(`document.querySelector('#settingsLogout').parentElement.classList.contains('my-avatar-settings')`),true);await evaluate(`document.querySelector('#mySettingsDialog').close()`);
  await evaluate(`document.querySelector('#xpHistoryButton').click()`);await delay(100);assert.equal(await evaluate(`document.querySelector('#xpHistoryDialog').open`),true);assert.equal(await evaluate(`document.querySelectorAll('#xpHistoryList [data-history-type="challenge"]').length`),1);await shot('xp-history');
  await evaluate(`document.querySelector('#xpHistoryList [data-history-type="challenge"]').click()`);await delay(150);assert.equal(await evaluate(`state.view`),'challenges');assert.equal(await evaluate(`document.querySelector('#challengeDetail').hidden`),false);
  await evaluate(`(async()=>{await showView('library');document.querySelector('#myGudongiTab').click()})()`);
  assert.equal(await evaluate(`(()=>{const host=document.createElement('section');host.className='admin-challenge-posts';host.innerHTML='<table class="admin-users"><tbody><tr><td id="alignmentProbe">제목<br>작성자</td><td>등록일</td><td>달성량</td></tr></tbody></table>';document.body.append(host);const value=getComputedStyle(document.querySelector('#alignmentProbe').nextElementSibling).verticalAlign;host.remove();return value})()`),'middle');
  ...
  assert.equal(await evaluate(`(async()=>{const response=await fetch('/__test/delete-preview-post',{method:'POST'});return response.status})()`),200);await delay(100);await evaluate(`window.refreshMyInfo()`);await delay(200);assert.equal(await evaluate(`document.querySelector('#deletionNoticeDialog').open`),true);assert.equal(await evaluate(`document.querySelector('#deletionNoticeReason').textContent`),'운영 기준에 맞지 않는 중복 기록입니다.');assert.match(await evaluate(`document.querySelector('#deletionNoticeXp').textContent`),/EXP가 회수/);await shot('deletion-notice');await evaluate(`document.querySelector('#deletionNoticeConfirm').click()`);await delay(100);assert.equal(await evaluate(`document.querySelector('#deletionNoticeDialog').open`),false);
  const adminSetup=await evaluate(`(async()=>{const response=await fetch('/api/admin/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:'admin',password:'test-admin-password'})}),data=await response.json();state.user='admin';applyAdminState(data);await showView('library');const profileResponse=await fetch('/api/account/gudongi'),profile=await profileResponse.json();if(profile.gudongi)window.applyGudongi(profile.gudongi);document.querySelector('#myGudongiTab').click();return {loginStatus:response.status,profileStatus:profileResponse.status}})()`);assert.deepEqual(adminSetup,{loginStatus:200,profileStatus:200});await delay(200);
  assert.equal(await evaluate(`document.querySelector('.gudongi-progress>div>span').textContent`),'EXP (50/50)');assert.equal(await evaluate(`document.querySelector('.gudongi-stats>div small').textContent`),'/ 10');
  await evaluate(`document.querySelector('#changeAppearance').click()`);await delay(150);assert.equal(await evaluate(`document.querySelectorAll('#appearanceGrid .appearance-choice').length`),10);assert.equal(await evaluate(`document.querySelector('[data-appearance="admin"] small').textContent`),'관리자 전용');assert.equal(await evaluate(`document.querySelector('[data-appearance="admin"] img').complete`),true);await shot('admin-gudongi');await evaluate(`document.querySelector('#appearanceDialog').close()`);
  assert.deepEqual(errors,[]);console.log(JSON.stringify({ok:true,output,checks:['header direct navigation','EXP history navigation','settings logout placement','1440/820/390 no overflow','rail spacing','appearance 3/2/1 columns','avatar crop/zoom/save/reload','appearance persistence','library tab','no browser exceptions','MAX labels','30 XP units with 8 filled','missing image fallback','keyboard tabs','admin deletion notice','admin 50/50 EXP and exclusive appearance']},null,2));
