...
export function fixture() {
  ...
  const env={ADMIN_SESSION_SECRET:'in-memory-test-only-not-production',ADMIN_PASSWORD:'test-admin-password',ADMIN_INITIAL_EMAIL:'admin@gmuc.or.kr',ADMIN_EMAIL_VERIFICATION_VERSION:'test-v1',CHALLENGE_FILES:{async put(key,bytes){objects.set(key,new Uint8Array(bytes));},async get(key){return objects.has(key)?{body:objects.get(key)}:null;},async delete(key){objects.delete(key);}},RANKINGS:{idFromName:name=>name,get:()=>store},ASSETS:{fetch:()=>new Response('Not found',{status:404})}};
