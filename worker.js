...
async function google(query,env){const url=new URL('https://www.googleapis.com/books/v1/volumes');url.searchParams.set('q',query);url.searchParams.set('printType','books');url.searchParams.set('langRestrict','ko');url.searchParams.set('maxResults','40');if(env.GOOGLE_BOOKS_API_KEY)url.searchParams.set('key',env.GOOGLE_BOOKS_API_KEY);const res=await fetch(url);if(!res.ok)throw new Error(`Google ${res.status}`);const data=await res.json();return (data.items||[]).map(item=>normalizeGoogle(item,query));}
async function naver(query,env){if(!env.NAVER_CLIENT_ID||!env.NAVER_CLIENT_SECRET)return [];const url=new URL('https://openapi.naver.com/v1/search/book.json');url.searchParams.set('query',query);url.searchParams.set('display','40');url.searchParams.set('sort','sim');const res=await fetch(url,{headers:{'X-Naver-Client-Id':env.NAVER_CLIENT_ID,'X-Naver-Client-Secret':env.NAVER_CLIENT_SECRET}});if(!res.ok)throw new Error(`Naver ${res.status}`);const data=await res.json();return (data.items||[]).map((item,index)=>normalizeNaver(item,index,query));}
