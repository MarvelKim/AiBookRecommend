...
- 로그인 사용자별 한국 시간 하루 추천 한도는 실제 제공된 30권이며, 성공 요청당 1 EXP를 하루 최대 4 EXP 적립합니다. 같은 요청 재시도와 후보 순환에는 중복 적립하지 않습니다.
- 챌린지는 서버에 저장된 `challenge_posts.achievement × 3`을 적립합니다. 달성량 수정·게시글 삭제에 따라 EXP와 해금 상태를 다시 계산합니다.
- 기존 데이터는 유지하고 `gudongi_*` 테이블·인덱스·트리거를 추가합니다. 기존 추천은 날짜별 최대 4 EXP, 기존 챌린지는 달성량 기준으로 한 번만 소급 반영하며 새 추천 권수 한도는 소비하지 않습니다.
- 제공된 캐릭터 원본 8개를 연결했습니다. LV 5(MAX)의 기본 모습은 정장 구동이입니다. LV 4 일상 원본은 아직 없어 `이미지 준비 중`으로 표시합니다. 이미지 로드 실패에도 대체 표시합니다.
...
운영 배포는 V6 화면 승인과 누락된 캐릭터 원본 준비 이후 기존 `npm run deploy` 절차로 별도 진행합니다.
